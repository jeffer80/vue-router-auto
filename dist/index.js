...
exports.default = (function (config) {
    ...
    var _a = Object.assign({}, defaultConfig, config), rc = _a.rc, redirect = _a.redirect, rootFile = _a.rootFile;
    if (rc === null)
        return Routers;
    // allRouters[object]：存储所有路由的变量：先将所有的路由生成，然后放在该变量里面
    var allRouters = {};
    // 通过循环RC(传入的文件)
    var routerFileAndLen = rc
        .keys()
        ...
        .sort(function (i, j) { return i.fileLen - j.fileLen; }); // 通过文件深度 升序排序
    // 传入文件中最大深度
    var maxLen = 0;
    ...
    var substrName = function (name) { return name.substr(0, name.lastIndexOf('-')); };
    /**
     ...
     */
    var ceateRouter = function (index, nofindnum, newcurr) {
        if (nofindnum === void 0) { nofindnum = 0; }
        if (newcurr === void 0) { newcurr = null; }
    ...
    var deleteNameFun = function (arr) {
        arr.map(function (r) {
            // 删除多余的fileName属性
            delete r.fileName;
            // 判断是否需要删除name属性
    ...
    // 若存在重定向的路由，则加入重定向
    if (redirect)
        Routers.unshift({ path: '/', redirect: redirect });
    // 返回正儿八经的的路由数据
    return Routers;
});
